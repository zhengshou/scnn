...
void* VideoDataLayerPrefetch(void* layer_pointer) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
	...
	//LOG(INFO) << "overlap " << datum.overlap();
...
void VideoDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK_LE(top->size(), 3) << "Data Layer takes at most three blobs as output.";	// 100515
  if (top->size() == 1) {
  ...
  std::ifstream infile(source.c_str()); // read each row in dir file
  int count = 0;
  string filename;
  ...
  // overlap 100515
  if (1) {
    (*top)[2]->Reshape(this->layer_param_.image_data_param().batch_size(), 1, 1, 1, 1);
...
Dtype VideoDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  caffe_copy(prefetch_data_->count(), prefetch_data_->cpu_data(),
			 ...
			 //LOG(INFO) << "output_labels_ " << output_labels_;
  if (output_labels_) {
