...
void HDF5DataLayer<Dtype>::LoadHDF5FileData(const char* filename) {
  ...
  hdf5_load_nd_dataset(
    file_id, this->layer_param_.top(0).c_str(),  MIN_DATA_DIM, MAX_DATA_DIM, &data_blob_);

  const int MIN_LABEL_DIM = 1;
  const int MAX_LABEL_DIM = INT_MAX;
  hdf5_load_nd_dataset(
    file_id, this->layer_param_.top(1).c_str(), MIN_LABEL_DIM, MAX_LABEL_DIM, &label_blob_);
