...
bool ReadImageSequenceToVolumeDatum(const char* img_dir, const int start_frm, const int label,
	...
	datum->set_length(length);
	datum->set_label(label);
	datum->set_overlap(overlap);	// 030317
	datum->clear_data();
	datum->clear_float_data();

	offset = 0;
	int end_frm = start_frm + length * sampling_rate;
	// LOG(INFO) << "img_dir: " << img_dir;	// 030317
	// LOG(INFO) << "start_frm: " << start_frm;	// 030317
	// LOG(INFO) << "end_frm: " << end_frm;	// 030317
	for (int i=start_frm; i<end_frm; i+=sampling_rate){
		...
		if (height > 0 && width > 0) {
		    ...
		    // 030717
		    if (!img_origin.data) {
				sprintf(fn_im, "%s/%06d.jpg", img_dir, i-sampling_rate);
				img_origin = cv::imread(fn_im, CV_LOAD_IMAGE_COLOR);
		    }
		    if (!img_origin.data) {
