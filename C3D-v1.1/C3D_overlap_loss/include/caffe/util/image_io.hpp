...
void BufferToGrayImage(const float* buffer, const int h, const int w, cv::Mat* img);
void BufferToGrayImage(const double* buffer, const int h, const int w, cv::Mat* img);
...
bool ReadImageSequenceToVolumeDatum(const char* img_dir, const int start_frm, const int label,
		const int length, const int height, const int width, const int sampling_rate, VolumeDatum* datum, const float overlap);	// 030317

inline bool ReadImageSequenceToVolumeDatum(const char* img_dir, const int start_frm, const int label,
		const int length, const int sampling_rate, VolumeDatum* datum, const float overlap){	// 030317
	return ReadImageSequenceToVolumeDatum(img_dir, start_frm, label, length, 0, 0, sampling_rate, datum, overlap);	// 030317
}
